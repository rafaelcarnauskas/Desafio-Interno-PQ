...
def net_analysis(df, THRESHOLD, window=21):
    ...
    df['A'] = np.nan()
    df['max_eigen_value'] = np.nan()
    df['norm_eigen_vector'] = np.nan()
    df['edges'] = np.nan()
    df['density'] = np.nan()


    for i in range(window, len(df['Adj Close']))
        corr = df['Adj Close'][i-window:i].corr()
        columns = df['Adj Close'].columns.tolist()
        ...
        df['A'][i] = A
        df['max_eigen_value'][i] = max_eigen_value
        df['norm_eigen_vector'][i] = norm_eigen_value
        df['edges'][i] = edges
        df['density'][i] = density
